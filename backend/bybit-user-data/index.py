...
def get_user_api_keys(user_id: int) -> tuple[str, str]:
    ...
    if not result:
        raise Exception('API keys not found for user')
...
def bybit_request(endpoint: str, api_key: str, api_secret: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    '''
    Получение реальных данных Bybit для авторизованного пользователя
    Args: event - HTTP запрос с X-User-Id заголовком и action параметром
    Returns: Данные аккаунта, позиций, ордеров с Bybit
    '''
            ...
            if action == 'balance':
                ...
                if result.get('retCode') == 0:
                    ...
                    return {
                        ...
                        'body': json.dumps({'success': True, 'data': formatted_orders}),
                        'isBase64Encoded': False
                    }
